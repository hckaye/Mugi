...
internal static class OpenApiImportGenerator
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var hintName = GeneratedNaming.StableIdentifier("Mugi.OpenApi.", input.Path) + ".g.cs";
        return new GenerationResult(
    ...
    private sealed class Builder
    {
            ...
            foreach (var property in propertyObject!.Properties)
            {
                ...
                if (!TryJsonPropertyIdentifier(property.Name, out var identifier))
                {
                    ReportUnrepresentable(
                        property.Position,
                        context + "." + property.Name,
                        "the JSON property name cannot be preserved by Mugi's JSON naming policy");
                    return null;
                }
        ...
        private void BuildPaths()
        {
            ...
            foreach (var pathProperty in paths!.Properties)
            {
                ...
                if (!TryConvertPath(pathProperty.Name, pathProperty.Position, out var mugiPath, out var routeNames))
                {
                    continue;
                }
            ...
            SimpleJsonObject operation)
        {
            ...
            _paths.Add(new KeyValuePair<string, string>(name, mugiPath));
            ...
            switch (source)
            {
                ...
                case "cookie":
                    ReportUnrepresentable(
                        ...
                        operationLabel + " parameter '" + name + "'",
                        "cookie parameters have no Mugi.Schema source");
            ...
            out bool bodyRequired)
        {
            ...
            if (mediaType is null)
            {
                ReportUnrepresentable(
                    content.Position,
                    operationLabel + " requestBody",
                    "only application/json request bodies can be mapped to Mugi.Schema");
                return _mode != OpenApiGenerationMode.Client;
            }
                ...
                && !string.Equals(bodySchemaType, "object", StringComparison.Ordinal))
            {
                ReportUnrepresentable(
                    effectiveBody.Position,
                    operationLabel + " requestBody",
                    "Mugi.Schema request bodies must be JSON objects");
                return false;
            }
            ...
            foreach (var constraintName in UnsupportedConstraintNames)
            {
                if (schema.TryGetValue(constraintName, out var unsupported))
                {
                    ReportUnrepresentable(
                        ...
                        "Mugi.Schema has no equivalent validation rule");
                }
            }
            ...
            out string? literal)
        {
            ...
            // Min and Max are the closest Mugi rules for exclusive floating-point bounds.
            return TryFormatNumber(type, value, out literal);
        }
        ...
        private void EmitSchemas(CodeWriter writer)
        {
            ...
            {
                writer.Line(
                    "public static readonly global::Mugi.Schema.Schema<" + operation.InputName + "> " +
                    operation.Name + " =");
                var schemaFactory =
                    "    global::Mugi.Schema.Schemas.For<" + operation.InputName + ">()";
            ...
            for (var index = 0; index < segments.Length; index++)
            {
                ...
                else if (segment.IndexOf('{') >= 0 || segment.IndexOf('}') >= 0)
                {
                    ReportUnrepresentable(
                        position,
                        path,
                        "Mugi route parameters must occupy a complete path segment");
                    return false;
                }
            }
