...
internal static class InvocationAnalyzer
{
        ...
        CancellationToken cancellationToken)
    {
        if (method.Name != "Param"
            ...
            || GetMetadataName(method.ContainingType) != "Mugi.Context"
        ...
        out SchemaPartDefinition? partDefinition)
    {
        ...
        if (method.Name is not ("For" or "Part")
            ...
            || GetMetadataName(method.ContainingType) != "Mugi.Schema.Schemas")
        {
            return false;
        }
               ...
               && memberAccess.Parent is InvocationExpressionSyntax next)
        {
            ...
            var expectedBuilderType = isPart
                ? "Mugi.Schema.SchemaPart`1"
                : "Mugi.Schema.Schema`1";
    ...
    private static bool TryGetSchemaPartUse(IMethodSymbol method, out ITypeSymbol? partType)
    {
        ...
        if (method.Name != "Use"
            || GetMetadataName(declaredMethod.ContainingType) != "Mugi.Schema.SchemaPartExtensions"
        ...
        out IMethodSymbol? method)
    {
        ...
        if (!(expression is InvocationExpressionSyntax invocation)
            ...
            || (GetMetadataName(target.ContainingType.OriginalDefinition) == "Mugi.Schema.Rule`1"
        ...
        out ImmutableArray<InvocationExpressionSyntax> invocations)
    {
        ...
        while (expression is InvocationExpressionSyntax invocation
               ...
               && GetMetadataName(method.ContainingType.OriginalDefinition) == "Mugi.Schema.Rule`1"
        ...
        out Diagnostic? diagnostic)
    {
        ...
        diagnostic = null;
        var declaredMethod = method.ReducedFrom ?? method;
        if (GetMetadataName(declaredMethod.ContainingType) != "Mugi.Schema.EndpointExtensions"
        ...
        out bool interceptJson)
    {
        ...
        if (method.Name == "Json" && containingName == "Mugi.Context" && method.Parameters.Length == 1)
        {
        ...
        if (method.Name == "Json" && containingName == "Mugi.Request" && method.Parameters.Length == 0)
        {
            jsonType = NormalizeTopLevelNullability(method.TypeArguments[0]);
            return true;
        }

        if (containingName == "Mugi.Json.Json"
    ...
    private static bool HasExplicitCodecParameter(IMethodSymbol method)
    {
        foreach (var parameter in method.Parameters)
        {
            if (parameter.Type is INamedTypeSymbol named
                && GetMetadataName(named.OriginalDefinition) == "Mugi.Json.IJsonCodec`1")
            {
                return true;
            }
        }
        ...
        out Diagnostic? diagnostic)
    {
        ...
        if (GetMetadataName(originalContainingType.OriginalDefinition) != "Mugi.App`1")
        {
            return false;
        }
    ...
    private static bool IsAnyRouteMethod(IMethodSymbol method)
    {
        ...
        return containingName == "Mugi.App`1"
            || containingName == "Mugi.Schema.EndpointExtensions";
    }
