...
internal static class ProgramEntry
{
    internal static async Task<int> Run(string[] args)
    {
        ...
        {
            Console.Error.WriteLine("mugi-gen: " + argumentError);
        ...
        {
            Console.Error.WriteLine("mugi-gen: project file does not exist: " + projectPath);
            return 2;
        }
        ...
        try
        {
            ...
            {
                Console.Error.WriteLine("mugi-gen: MSBuild did not produce a compilation for " + projectPath);
            ...
            if (compilationErrors.Count != 0)
            {
                ...
                Console.Error.WriteLine(
                    "mugi-gen: the project does not compile, so generation would be incomplete. " +
                    "Restore and build the project first (dotnet build), then re-run mugi-gen.");
                return 6;
            }
            ...
            else
            {
                ...
                {
                    Console.Error.WriteLine("mugi-gen: generation failed because the project contains Mugi generator errors.");
                    return 4;
                }
        ...
        {
            Console.Error.WriteLine("mugi-gen: " + exception.Message);
            return 5;
        }
    }
    ...
    private static async Task<int> RunImport(string[] args)
    {
        ...
        {
            Console.Error.WriteLine("mugi-gen: " + error);
            Console.Error.WriteLine(
                "Usage: mugi-gen import --input <openapi.json> --output <directory> [--namespace <namespace>]");
            return 2;
        }
        ...
        {
            Console.Error.WriteLine("mugi-gen: OpenAPI document does not exist: " + inputPath);
            return 2;
        }
        ...
        try
        {
            ...
            if (result.Diagnostics.Any(static diagnostic => diagnostic.Severity == DiagnosticSeverity.Error))
            {
                Console.Error.WriteLine("mugi-gen: import failed because the OpenAPI document has errors.");
                return 4;
            }
            ...
            for (var index = 0; index < sources.Length; index++)
            {
                ...
                var destination = Path.Combine(outputPath, "Mugi.OpenApi." + baseName + suffix + ".g.cs");
    ...
    private static async Task<int> RunClient(string[] args)
    {
                ...
                out var error))
        {
            Console.Error.WriteLine("mugi-gen: " + error);
            Console.Error.WriteLine(
                "Usage: mugi-gen client --input <openapi.json> --output <directory> "
        ...
        try
        {
            ...
            if (result.Diagnostics.Any(static diagnostic => diagnostic.Severity == DiagnosticSeverity.Error))
            {
                Console.Error.WriteLine("mugi-gen: client generation failed because the OpenAPI document has errors.");
                return 4;
            }
            ...
            for (var index = 0; index < result.Sources.Length; index++)
            {
                ...
                var destination = Path.Combine(outputPath, "Mugi.OpenApiClient." + baseName + suffix + ".g.cs");
    ...
    {
        var prefix = "Mugi.OpenApiClient." + baseName;
    ...
    private static ProjectSettings ReadProjectSettings(string projectPath, Project project)
    {
        ...
        {
            naming = ParseNaming(evaluatedProject.GetPropertyValue("MugiJsonNaming"));
        }
    ...
    private static void WriteUsage(bool openApi)
    {
        Console.Error.WriteLine(openApi
            ? "Usage: mugi-gen openapi --project <project.csproj> --output <openapi.json>"
            : "Usage: mugi-gen --project <project.csproj> --output <directory>");
    }
    ...
    private static void WriteWorkspaceFailures(IEnumerable<string> failures)
    {
        ...
        {
            Console.Error.WriteLine("mugi-gen: MSBuild workspace: " + failure);
        }
    }
