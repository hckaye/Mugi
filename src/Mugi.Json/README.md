...
Mugi.Json is the NativeAOT-compatible JSON serializer used by the [Mugi](https://www.nuget.org/packages/Mugi) web framework. It reads and writes JSON through per-type codecs registered ahead of time, so serialization works without reflection or runtime code generation.
