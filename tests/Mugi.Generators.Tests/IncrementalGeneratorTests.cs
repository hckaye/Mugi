...
public sealed class IncrementalGeneratorTests
{
    ...
    public void Unrelated_edit_reuses_json_type_pipeline_output()
    {
        ...
        var jsonTypeSteps = result.TrackedSteps["MugiJsonTypes"];
        var jsonSourceSteps = result.TrackedSteps["MugiJsonSources"];
        ...
        Assert.All(
            ...
            static output => Assert.True(
                output.Reason is IncrementalStepRunReason.Cached or IncrementalStepRunReason.Unchanged));
        Assert.Equal(
            first.SourcesWithPrefix("Mugi.JsonCodec."),
            string.Join(
                Environment.NewLine,
                driver.GetRunResult().Results.Single().GeneratedSources
                    .Where(static sourceResult => sourceResult.HintName.StartsWith("Mugi.JsonCodec.", StringComparison.Ordinal))
    ...
    public void Editing_one_type_keeps_other_type_outputs_cached_or_unchanged()
    {
        ...
        AssertTrackedSourceUnchanged(result, "MugiJsonSources", "Second");
    }
    ...
    public void Editing_one_route_keeps_other_route_and_interceptor_outputs_cached_or_unchanged()
    {
        ...
        AssertTrackedSourceUnchanged(result, "MugiRouteSources", "_002F_b");
        AssertTrackedSourceUnchanged(result, "MugiInterceptorSources", "SecondRoutes");
    }
    ...
    public void Editing_a_part_updates_consumers_and_keeps_unrelated_schemas_unchanged()
    {
        ...
        var driver = first.Driver.RunGeneratorsAndUpdateCompilation(updated, out _, out _);
        var result = driver.GetRunResult().Results.Single();
        var consumerOutputs = result.TrackedSteps["MugiSchemaSources"]
        ...
        AssertTrackedSourceUnchanged(result, "MugiSchemaSources", "UnrelatedInput");
    }
    ...
    public void Unrelated_source_edit_reuses_openapi_import_output()
    {
        ...
        Assert.All(
            result.TrackedSteps["MugiOpenApiInputs"].SelectMany(static step => step.Outputs),
            static output => Assert.True(
                output.Reason is IncrementalStepRunReason.Cached or IncrementalStepRunReason.Unchanged));
        Assert.All(
            result.TrackedSteps["MugiOpenApiSources"].SelectMany(static step => step.Outputs),
            static output => Assert.True(
                output.Reason is IncrementalStepRunReason.Cached or IncrementalStepRunReason.Unchanged));
    }
    ...
    public void Openapi_content_edit_updates_openapi_import_output()
    {
        ...
        var generated = string.Join(
            Environment.NewLine,
            driver.GetRunResult().Results.Single().GeneratedSources
                .Where(static source => source.HintName.StartsWith("Mugi.OpenApi.", StringComparison.Ordinal))
                .Select(static source => source.SourceText.ToString()));
