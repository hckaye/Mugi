...
public sealed class OpenApiClientTests
{
    ...
    public void Representative_document_generates_client_operations_and_codecs()
    {
        ...
        Assert.Contains("global::Mugi.Json.Json.Serialize(bodyBuffer, body)", source, StringComparison.Ordinal);
    ...
    [Fact]
    public void Generated_client_compiles_with_only_mugi_json_reference()
    {
        var result = Generate(RepresentativeDocument(), "Demo.Api", "PetStoreClient");
        var source = result.Sources.Single().Source;
    ...
    public void Client_metadata_can_be_enabled_without_server_import_metadata()
    {
        ...
        var run = GeneratorTestHelper.Run(
            ...
            additionalFileMetadata: new Dictionary<string, IReadOnlyDictionary<string, string>>
            {
                [path] = new Dictionary<string, string>(StringComparer.Ordinal)
                {
                    ...
                    ["MugiOpenApiNamespace"] = "Demo.Client",
        ...
        Assert.Contains("public sealed class CatalogClient", run.SourcesWithPrefix("Mugi.OpenApiClient."), StringComparison.Ordinal);
        Assert.Contains("public sealed class ApiException", run.SourcesWithPrefix("Mugi.OpenApiClient.ApiException."), StringComparison.Ordinal);
        Assert.Empty(run.SourcesWithPrefix("Mugi.OpenApi."));
    }
    ...
    public void Client_and_server_metadata_can_share_one_document()
    {
        ...
        var run = GeneratorTestHelper.Run(
            ...
            additionalFileMetadata: new Dictionary<string, IReadOnlyDictionary<string, string>>
            {
                [path] = new Dictionary<string, string>(StringComparer.Ordinal)
                {
                    ["MugiOpenApi"] = "true",
                    ["MugiOpenApiClient"] = "true",
                    ["MugiOpenApiNamespace"] = "Demo.Shared",
                    ["MugiOpenApiClientName"] = "SharedClient",
                },
            });
        ...
        Assert.DoesNotContain(
            run.DriverDiagnostics.Concat(run.Result.Diagnostics).Concat(run.Compilation.GetDiagnostics()),
            static diagnostic => diagnostic.Severity == DiagnosticSeverity.Error);
        Assert.Contains("public static partial class Paths", run.SourcesWithPrefix("Mugi.OpenApi."), StringComparison.Ordinal);
        Assert.Contains("public sealed class SharedClient", run.SourcesWithPrefix("Mugi.OpenApiClient."), StringComparison.Ordinal);
        Assert.DoesNotContain(
            "public sealed record Item(",
            run.SourcesWithPrefix("Mugi.OpenApiClient."),
            StringComparison.Ordinal);
    }
    ...
    public void Client_metadata_uses_root_namespace_when_no_namespace_is_configured()
    {
        ...
        Assert.DoesNotContain(
            run.DriverDiagnostics.Concat(run.Result.Diagnostics).Concat(run.Compilation.GetDiagnostics()),
            static diagnostic => diagnostic.Severity == DiagnosticSeverity.Error);
        Assert.Contains("namespace Demo.Default", run.SourcesWithPrefix("Mugi.OpenApiClient."), StringComparison.Ordinal);
        Assert.Contains("public sealed class PetStoreClient", run.SourcesWithPrefix("Mugi.OpenApiClient."), StringComparison.Ordinal);
        Assert.Contains("public sealed class ApiException", run.SourcesWithPrefix("Mugi.OpenApiClient.ApiException."), StringComparison.Ordinal);
    }
    ...
    public void Cookie_and_non_json_operations_are_skipped_with_existing_diagnostics()
    {
        ...
        Assert.Contains(result.Diagnostics, diagnostic => diagnostic.Id == "MUGI022");
        var source = result.Sources.Single().Source;
    ...
    public void Incompatible_success_response_schemas_report_diagnostic_and_skip_operation()
    {
        ...
        var diagnostic = Assert.Single(result.Diagnostics, diagnostic => diagnostic.Id == "MUGI022");
    ...
    private static object? TryGetCodec(Type type) => typeof(Mugi.Json.Json)
        .GetMethod(nameof(Mugi.Json.Json.TryGetCodec), BindingFlags.Public | BindingFlags.Static)!
    ...
    private static ImmutableArray<MetadataReference> JsonOnlyReferences()
    {
        ...
        return runtime
            .Append(typeof(Mugi.Json.Json).Assembly.Location)
