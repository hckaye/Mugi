...
{
    [Theory]
    [InlineData("MUGI001", "var context = new Context(); context.Json(new { Value = 1 });")]
    [InlineData("MUGI002", "var app = new App(); app.Get(\"missing-slash\", c => c.Text(\"x\"));")]
    [InlineData("MUGI004", "var context = new Context(); context.Json<object>(new object());")]
    ...
    public void Sequential_duplicate_on_same_local_reports_warning()
    {
        ...
        Assert.Contains(run.Result.Diagnostics, diagnostic => diagnostic.Id == "MUGI003");
    }
    ...
    public void Explicit_codec_overloads_do_not_request_generated_object_codecs()
    {
        ...
        Assert.DoesNotContain(run.Result.Diagnostics, diagnostic => diagnostic.Id == "MUGI004");
    ...
    public void Source_record_without_primary_constructor_remains_unsupported()
    {
        ...
        var diagnostic = Assert.Single(run.Result.Diagnostics, diagnostic => diagnostic.Id == "MUGI004");
    ...
    public void Reordered_named_route_arguments_are_mapped_to_their_parameters()
    {
        ...
        Assert.DoesNotContain(run.Result.Diagnostics, diagnostic => diagnostic.Id == "MUGI002");
        Assert.Contains("/get/:id", run.SourcesWithPrefix("Mugi.RouteTemplate."), StringComparison.Ordinal);
        Assert.Contains("/on/:id", run.SourcesWithPrefix("Mugi.RouteTemplate."), StringComparison.Ordinal);
        Assert.DoesNotContain(
            run.Compilation.GetDiagnostics(),
            diagnostic => diagnostic.Severity == Microsoft.CodeAnalysis.DiagnosticSeverity.Error);
    }
    ...
    public void Mutually_exclusive_route_registrations_do_not_report_duplicates()
    {
        ...
        Assert.DoesNotContain(run.Result.Diagnostics, diagnostic => diagnostic.Id == "MUGI003");
    }
}
