...
public sealed class SchemaRuleMethodGeneratorTests
{
    ...
    [Fact]
    public void Multi_statement_rule_method_reports_Mugi025()
    {
        const string source = """
            using Mugi.Schema;
    ...
    [Fact]
    public void Instance_rule_method_reports_Mugi025()
    {
        const string source = """
            using Mugi.Schema;
            internal sealed record Input(string Value);
    ...
    [Fact]
    public void Cross_assembly_rule_method_reports_Mugi025()
    {
        ...
        AssertDiagnostic(run, "MUGI025", RuleDeclarationMessage);
    }

    [Fact]
    public void Rule_chain_not_rooted_at_the_parameter_reports_Mugi025()
    {
        const string source = """
            using Mugi.Schema;
            internal sealed record Input(string Value);
    ...
    [Fact]
    public void Conditional_inline_rules_report_Mugi025_even_with_another_schema_error()
    {
        const string source = """
            using Mugi.Schema;
            internal sealed record Input(string Value);
        ...
        Assert.Contains(run.Result.Diagnostics, diagnostic => diagnostic.Id == "MUGI013");
        AssertDiagnostic(run, "MUGI025", RuleDeclarationMessage);
    }

    [Fact]
    public void Private_predicate_reports_Mugi026_without_generated_access_error()
    {
        const string source = """
            using Mugi;
            using Mugi.Schema;
        ...
        var run = GeneratorTestHelper.Run(GeneratorTestHelper.CreateCompilation(source));
        var diagnostic = Assert.Single(run.Result.Diagnostics, diagnostic => diagnostic.Id == "MUGI026");
