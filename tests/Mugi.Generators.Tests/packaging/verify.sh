...
dotnet pack "$repository_root/src/Mugi.Json/Mugi.Json.csproj" -c Release \
    -p:PackageVersion=1.0.0 -p:PackageOutputPath="$packages_dir"
dotnet pack "$repository_root/src/Mugi/Mugi.csproj" -c Release \
    -p:PackageVersion=1.0.0 -p:PackageOutputPath="$packages_dir"
dotnet pack "$repository_root/src/Mugi.Generators/Mugi.Generators.csproj" -c Release \
    -p:PackageVersion=1.0.0 -p:PackageOutputPath="$packages_dir"

unzip -l "$packages_dir/Mugi.Generators.1.0.0.nupkg" | grep 'analyzers/dotnet/cs/Mugi.Generators.dll'
if unzip -l "$packages_dir/Mugi.Generators.1.0.0.nupkg" | grep 'analyzers/dotnet/cs/Mugi.Generators.Core.dll'; then
    echo "Mugi.Generators must contain a single analyzer assembly." >&2
    exit 1
fi
unzip -l "$packages_dir/Mugi.Generators.1.0.0.nupkg" | grep 'buildTransitive/Mugi.Generators.props'
if unzip -p "$packages_dir/Mugi.Generators.1.0.0.nupkg" '*.nuspec' | grep 'Microsoft.CodeAnalysis'; then
    echo "Mugi.Generators must not expose Roslyn package dependencies." >&2
    exit 1
fi
...
find "$script_dir/DirectLibrary/obj/generated" -name 'Mugi.Interceptor.*.g.cs' -print | grep .
find "$script_dir/TransitiveApp/obj/generated" -name 'Mugi.Interceptor.*.g.cs' -print | grep .
