...
public sealed class SchemaGeneratorTests
{
    ...
    public void Schema_generates_registered_binder_and_body_codec()
    {
        ...
        Assert.Contains(
            ...
            "global::Miya.Schema.SchemaRegex.Create(\"^[A-Z]\");",
            generated,
    ...
    public void Schema_without_form_keeps_form_loading_out_of_generated_code()
    {
        ...
        Assert.Equal(
            first.SourcesWithPrefix("Miya.SchemaBinder."),
            second.SourcesWithPrefix("Miya.SchemaBinder."));
        Assert.DoesNotContain("context.Req.Form()", first.SourcesWithPrefix("Miya.SchemaBinder."), StringComparison.Ordinal);
    }
    ...
    public void Pascal_case_naming_is_used_for_validation_error_fields()
    {
        ...
        Assert.DoesNotContain(AllDiagnostics(run), diagnostic => diagnostic.Severity == DiagnosticSeverity.Error);
        Assert.Contains(
            "new global::Miya.Schema.ValidationError(\"UserId\", \"has an invalid value\")",
            generated,
            StringComparison.Ordinal);
        Assert.Contains(
            "new global::Miya.Schema.ValidationError(\"UserId\", \"is required\")",
            generated,
            StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void Miya016_reports_form_and_body_mappings_in_one_schema()
    {
        const string source = """
            using Miya;
            using Miya.Schema;
        ...
        Assert.Contains(AllDiagnostics(run), diagnostic => diagnostic.Id == "MIYA016");
    }
    ...
    public void FormFile_selector_reports_unsupported_schema_field_type()
    {
        ...
        Assert.Contains(
            AllDiagnostics(run),
            diagnostic => diagnostic.Id == "MIYA012" && diagnostic.GetMessage().Contains("form", StringComparison.Ordinal));
    }
