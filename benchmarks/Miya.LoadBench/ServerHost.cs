    ...
    private static async Task RunMiyaAsync(int port)
    {
        ...
        app.Post("/echo", static async context =>
        {
            var payload = await context.Req.Json<EchoPayload>().ConfigureAwait(false)
                ?? throw new Miya.Json.JsonException("An echo payload is required.");
