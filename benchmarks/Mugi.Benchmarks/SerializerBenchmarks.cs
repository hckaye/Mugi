...
{
    private ArrayBufferWriter<byte> _mugiBuffer = null!;
    ...
    public void Setup()
    {
        ...
        _mugiBuffer = new ArrayBufferWriter<byte>(16);
    ...
    public int SmallDtoMugi() => SerializeMugi(_small);
    ...
    public int List100Mugi() => SerializeMugi(_items);
    ...
    public int NestedMugi() => SerializeMugi(_nested);
    ...
    public int EscapeHeavyMugi() => SerializeMugi(_escapeHeavy);
    ...
    public int LongStringMugi() => SerializeMugi(_longString);
    ...
    public int IntegersMugi() => SerializeMugi(_integers);
    ...
    public RequestDto RequestBindMugi() => global::Mugi.Json.Json.Deserialize<RequestDto>(_requestJson)!;
    ...
    public int BufferGrowthMugi()
    {
        var buffer = new ArrayBufferWriter<byte>(16);
        global::Mugi.Json.Json.Serialize(buffer, _longString);
    ...
    private int SerializeMugi<T>(T value)
    {
        _mugiBuffer.Clear();
        global::Mugi.Json.Json.Serialize(_mugiBuffer, value);
        return _mugiBuffer.WrittenCount;
    }
    ...
    {
        var mugi = new ArrayBufferWriter<byte>();
        var stj = new ArrayBufferWriter<byte>();
        global::Mugi.Json.Json.Serialize(mugi, value);
        ...
        using var mugiDocument = JsonDocument.Parse(mugi.WrittenMemory);
        using var stjDocument = JsonDocument.Parse(stj.WrittenMemory);
        if (!JsonElement.DeepEquals(mugiDocument.RootElement, stjDocument.RootElement))
        {
    ...
    {
        global::Mugi.Json.Json.Include<SmallDto>();
        // STJ does not enforce nullable annotations for collection elements, so match that contract.
        global::Mugi.Json.Json.Include<List<ItemDto?>>();
        global::Mugi.Json.Json.Include<NestedDto>();
        global::Mugi.Json.Json.Include<string>();
        global::Mugi.Json.Json.Include<IntegerPayload>();
        global::Mugi.Json.Json.Include<RequestDto>();
    }
    ...
    private static void ValidateMugiRejects(ReadOnlySpan<byte> json)
    {
        try
        {
            _ = global::Mugi.Json.Json.Deserialize<RequestDto>(json);
        }
        catch (global::Mugi.Json.JsonException exception) when (exception.IsInputError)
        {
            return;
        }

        throw new InvalidOperationException("Mugi accepted input that violates the benchmark contract.");
    }
