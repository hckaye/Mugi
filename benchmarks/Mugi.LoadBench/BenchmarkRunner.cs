...
internal sealed class BenchmarkRunner
{
    ...
    [
        ...
        new("json-get", HttpMethod.Get, "/users/123", "{\"id\":\"123\",\"name\":\"Mugi\"}"),
        new("json-post", HttpMethod.Post, "/echo", Encoding.UTF8.GetString(EchoBody)),
    ];
    private static readonly string[] Frameworks = ["mugi", "aspnet"];
    ...
    public async Task RunAsync()
    {
        ...
        foreach (var scenario in Scenarios)
        {
            foreach (var framework in Frameworks)
            {
                var displayName = framework == "aspnet" ? "ASP.NET Core" : "Mugi";
    ...
    private async Task<BenchmarkResult> RunScenarioAsync(string framework, Scenario scenario)
    {
        ...
        return new BenchmarkResult(
            scenario.Name,
            framework == "aspnet" ? "ASP.NET Core" : "Mugi",
